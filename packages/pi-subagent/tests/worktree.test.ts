...
describe("WorktreeManager", () => {
  ...
  it("sweep reclaims over-worktrees with unique commits: patch archived, branch preserved", async () => {
    ...
    // Lifecycle GC: directory reclaimed immediately once the run is over.
    expect(report.removed).toContain(committed.cwd);
    await expect(fs.stat(committed.cwd)).rejects.toThrow();
    // Meaningful impact preserved: applyable patch + branch with the commits.
    expect(report.archived).toHaveLength(1);
    const patch = await fs.readFile(report.archived[0]!, "utf8");
    expect(patch).toContain("work.txt");
    expect(patch).toContain("important");
    const branch = await run("git", ["show-ref", "--verify", `refs/heads/${committed.branch}`], repo);
    expect(branch.code).toBe(0);
  });

  it("archived patches from swept worktrees re-apply onto the base repo", async () => {
    const dirty = await mgr.create(repo, "dirty-archive");
    await fs.writeFile(path.join(dirty.cwd, "uncommitted.txt"), "agent work\n");
    const old = new Date(Date.now() - 3 * 24 * 60 * 60_000);
    await fs.utimes(dirty.cwd, old, old);

    const report = await mgr.sweep(repo, 7, new Set());
    expect(report.removed).toContain(dirty.cwd);
    expect(report.archived).toHaveLength(1);

    const applied = await mgr.applyArchivedPatch(report.archived[0]!, repo);
    expect(applied.applied).toBe(true);
    expect(await fs.readFile(path.join(repo, "uncommitted.txt"), "utf8")).toBe("agent work\n");
  });
  ...
  it("sweep removes over-worktrees (archiving changed ones) and shields live worktrees", async () => {
    ...
    // Unchanged orphan: plain removal, nothing to archive.
    expect(report.removed).toContain(stale.cwd);
    await expect(fs.stat(stale.cwd)).rejects.toThrow();
    // Changed orphan: archived then removed — no wall-clock retention.
    expect(report.removed).toContain(changed.cwd);
    await expect(fs.stat(changed.cwd)).rejects.toThrow();
    expect(report.archived).toHaveLength(1);
