...
export function isWellFormedOp(op) {
    if (!Number.isSafeInteger(op.lamport) || op.lamport < 0 || op.lamport > MAX_LAMPORT)
        return false;
    if (op.entry_id === "" || op.origin_peer === "")
        return false;
    if (op.op === "upsert") {
        const fields = op.payload.fields;
        if (fields === undefined)
            return false;
        if (typeof fields.text !== "string" || fields.text.length > MAX_OP_TEXT)
            return false;
        ...
        if (fields.scope === "project" && (fields.project_id ?? null) === null)
            return false;
        if (fields.scope === "global" && (fields.project_id ?? null) !== null)
            return false;
    }
    if (op.op === "edit") {
        const text = op.payload.text;
        if (typeof text !== "string" || text.length > MAX_OP_TEXT)
            return false;
    }
    if (op.op === "confirm") {
...
export function compareOps(a, b) {
    ...
    if (a.origin_peer !== b.origin_peer)
        return a.origin_peer < b.origin_peer ? -1 : 1;
    if (a.id !== b.id)
 ...
 */
export function applyOp(entry, op) {
    /**
    ...
    switch (op.op) {
        ...
        case "edit": {
            ...
            if (op.lamport < entry.lamport)
                return entry;
            const { text, text_norm } = op.payload;
            return {
                ...entry,
                text,
                text_norm,
...
export function manualProvenance(by, at, chatId = null) {
    return {
