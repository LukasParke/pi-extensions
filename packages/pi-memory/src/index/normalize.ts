...
export function cosine(a: Float32Array, b: Float32Array): number {
	if (a.length !== b.length) {
		throw new Error(
			`vector length mismatch: ${String(a.length)} vs ${String(b.length)} — the index was written by a different model`,
		);
	}
	let acc = 0;
...
export function isNearDuplicate(
	...
	aVec: Float32Array,
	bVec: Float32Array,
): { duplicate: boolean; cosine: number; jaccard: number } {
...
export function fuseRankings(rankings: readonly string[][], k = RRF_K): Map<string, number> {
