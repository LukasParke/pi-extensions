 ...
 */
export function parseCandidates(raw: string): ExtractionResult {
	...
	for (const item of parsed) {
		...
		if (verdict.ok) out.push(verdict.candidate);
		else rejected.push({ text: preview(item), reason: verdict.reason });
	}

	return { candidates: out, rejected };
}

function validate(item: unknown): { ok: true; candidate: Candidate } | { ok: false; reason: string } {
	if (item === null || typeof item !== "object") return { ok: false, reason: "not an object" };
	const o = item as { text?: unknown; scope?: unknown };
