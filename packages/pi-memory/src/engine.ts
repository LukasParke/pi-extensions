...
export class MemoryEngine {
	...
	constructor(readonly opts: MemoryEngineOptions) {
	...
	async recall(
		...
		context: ScopeFilter,
		limit = 8,
	): Promise<{ hits: RecallHit[]; tookMs: number }> {
		...
		const dense: { id: string; cosine: number }[] = [];
		...
		for (const [id, score] of fused) {
			...
			if (entry === null || entry.tombstone) continue;
			 ...
			 * indefensible.
			 */
			if (cos !== null && cos < DENSE_GATE && (sRank === null || sRank > 3)) continue;
	...
	async add(input: {
	...
	}): Promise<{ entry: MemoryEntry; deduped: boolean } | null> {
		 ...
		 */
		if (redacted === "" || /^(«redacted:[^»]*»[\s,.:;-]*)+$/.test(redacted)) return null;
		if (redacted.length > MAX_TEXT_LENGTH) return null;
		...
		if (exact !== null) return { entry: this.confirm(exact.id), deduped: true };
		...
		for (const candidate of nearest) {
			...
			if (verdict.duplicate) return { entry: this.confirm(candidate.entry.id), deduped: true };
		}
		...
		const entry = this.store.applyLocal(
			...
			(existing) => applyOp(existing, lastOp(this.store, entryId)),
			vector,
		);
	 ...
	 */
	async update(id: string, text: string): Promise<MemoryEntry | null> {
		const existing = this.store.get(id);
		if (existing === null || existing.tombstone) return null;

		const redacted = this.redact(text).trim();
		if (redacted === "" || redacted.length > MAX_TEXT_LENGTH) return null;
		const textNorm = normalize(redacted);
		if (textNorm === "") return null;
		...
		for (const candidate of this.nearestInScope(vector, existing.scope, existing.project_id, 5)) {
			if (candidate.entry.id === id) continue;
			const verdict = isNearDuplicate(redacted, candidate.entry.text, vector, candidate.vector);
	...
	forget(id: string): boolean {
		...
		if (existing.tombstone) return true;

		this.store.applyLocal(
			{ id: ulid(this.now()), entry_id: id, op: "forget", lamport: this.clock.tick(), payload: {} },
			(current) => applyOp(current, lastOp(this.store, id)),
		);
		return true;
	}
	...
	forgetAllFromChat(chatId: string): number {
		...
		let n = 0;
		for (const id of ids) if (this.forget(id)) n++;
		return n;
	}

	private confirm(id: string): MemoryEntry {
		...
		return result ?? (this.store.get(id) as MemoryEntry);
	}
	...
	private nearestInScope(
		...
		k: number,
	): { entry: MemoryEntry; vector: Float32Array }[] {
		const filter: ScopeFilter =
			scope === "global" ? { mode: "global" } : { mode: "project", project_id: projectId };
		const eligible = new Map(this.store.list(filter, { limit: 100_000 }).map((e) => [e.id, e]));

		const scored: { entry: MemoryEntry; vector: Float32Array; cos: number }[] = [];
		for (const [id, vec] of this.store.liveVectors()) {
	...
	stats(): { total: number; global: number; by_project: { project_id: string; n: number }[] } {
		...
		for (const batch of chunk(live, 32)) {
			...
			for (const [i, entry] of batch.entries()) {
				...
				this.store.applyLocal(
					...
					(current) => applyOp(current, lastOp(this.store, entry.id)),
					v,
				);
			}
		}
	...
	mergePreview(incoming: readonly MemoryLogRecord[]): { digest: string; entry_count: number } {
	 ...
	 * than a loss.
	 */
	async backfillVectors(limit = 500): Promise<number> {
	 ...
	 */
	list(filter: ScopeFilter, opts: { limit?: number } = {}): MemoryEntry[] {
 ...
 */
function foldEntry(store: MemoryStore, entryId: string, incoming: MemoryLogRecord): MemoryEntry | null {
 ...
 */
function lastOp(store: MemoryStore, entryId: string): MemoryLogRecord {
	const log = store.readLog({ limit: 100_000 });
	const forEntry = log.filter((o) => o.entry_id === entryId);
	const last = forEntry[forEntry.length - 1];
	if (last === undefined) throw new Error(`no op for entry ${entryId}`);
	return last;
}
