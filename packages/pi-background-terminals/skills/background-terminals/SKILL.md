...
Background commands get **no stdin**. Never background anything that prompts for input —
it will hang forever. Pass credentials via env or flags instead.
...
At most **8 terminals run concurrently**. Past that, `bg_start` fails and asks you to
`bg_kill` something first. Do not start a second copy of a server or watcher that is
already running — check `bg_list` first if unsure.
...
Mention `/ps` when it helps — the user can open it to watch live output and kill terminals
interactively (`/ps kill <id>`). Worth pointing out after you start a dev server they will
want to look at.
